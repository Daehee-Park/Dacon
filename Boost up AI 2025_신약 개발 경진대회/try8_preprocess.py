...
def featurize_dataframe(df: pd.DataFrame, smiles_col='Canonical_Smiles'):
    ...
    # 4-B compute count-based fingerprints (uint16)
...
# ------------------------- 6. Feature Selection -----------------------------
print('Skipping variance-based feature selection, using all fingerprint features.')

# Define column types
scalar_cols = list(COMPREHENSIVE_DESCRIPTOR_FUNCS.keys())
fp_cols = [c for c in X_train_raw.columns if c.startswith('fp_')]

# We are keeping all fingerprint columns for the next steps
all_feature_cols = scalar_cols + fp_cols
X_train_pre_scaled = X_train_raw[all_feature_cols]
X_test_pre_scaled = X_test_raw[all_feature_cols]


# ------------------------- 7. Scaling ---------------------------------------
# By convention fingerprints are left untouched; only scale the scalar columns
scaler = StandardScaler()
X_train_scaled_scalar = pd.DataFrame(
        scaler.fit_transform(X_train_pre_scaled[scalar_cols]),
        columns=scalar_cols,
        index=X_train_pre_scaled.index)

X_test_scaled_scalar = pd.DataFrame(
        scaler.transform(X_test_pre_scaled[scalar_cols]),
        columns=scalar_cols,
        index=X_test_pre_scaled.index)

# Reassemble: scaled scalar part + selected fingerprints
X_train_scaled = pd.concat([X_train_scaled_scalar, X_train_pre_scaled[fp_cols]], axis=1)
X_test_scaled  = pd.concat([X_test_scaled_scalar,  X_test_pre_scaled[fp_cols]], axis=1)
...
os.makedirs('./output/try8', exist_ok=True)
X_train.to_csv('./output/try8/preprocessed_train.csv', index=False)
X_test .to_csv('./output/try8/preprocessed_test.csv' , index=False)
...
with open('./output/try8/feature_selection_info.json', 'w') as f:
    json.dump(feature_info, f, indent=2)

print("Preprocessing completed. Files saved to './output/try8/'.")