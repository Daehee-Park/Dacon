...
print("=== Original Data Basic Preprocessing ===")
...
# 데이터 저장
X_train_processed.to_csv(RESULT_PATH + 'X_train_original.csv', index=False)
X_test_processed.to_csv(RESULT_PATH + 'X_test_original.csv', index=False)
y_train.to_csv(RESULT_PATH + 'y_train.csv', index=False)
test_ids.to_csv(RESULT_PATH + 'test_ids.csv', index=False)

print(f"\nOriginal data preprocessing complete!")
print(f"Features: {X_train_processed.shape[1]}")
print(f"Files saved with '_original' suffix") 