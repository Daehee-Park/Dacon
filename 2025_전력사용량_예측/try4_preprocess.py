print('try4_preprocess.py is running')
import pandas as pd
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('output/try4/preprocess.log', encoding='utf-8')
    ]
)
...
train_processed.to_csv('output/try4/preprocessed_train.csv', index=False)

test_processed = pd.DataFrame(X_test_processed, columns=preprocessor.get_feature_names_out())
test_processed.to_csv('output/try4/preprocessed_test.csv', index=False)

logger.info("Saved preprocessed data to output/try4/")
