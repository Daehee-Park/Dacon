...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('output/preprocess.log', encoding='utf-8')
    ]
)
...
train_processed.to_csv('output/try1/preprocessed_train.csv', index=False)

test_processed = pd.DataFrame(X_test_processed, columns=preprocessor.get_feature_names_out())
test_processed.to_csv('output/try1/preprocessed_test.csv', index=False)

logger.info("Saved preprocessed data to output/try1/")
